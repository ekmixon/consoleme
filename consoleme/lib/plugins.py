...
def get_plugin_by_name(plugin_name: str) -> Any:
    ...
    exception_message = f"{initial_exception_message}Plugins found: {', '.join(plugins)}. Make sure you've set the environment variable CONSOLEME_CONFIG_ENTRYPOINT to the name of your configuration entrypoint, otherwise it will default to `default_config`."
...
def import_class_by_name(class_full_path: str):
    ...
    class_name = class_full_path[d + 1:]
    m = __import__(class_full_path[:d], globals(), locals(), [class_name])
